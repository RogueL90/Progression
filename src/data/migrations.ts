...
import { recoverOrphanedProjectPhotos } from '@/data/photoRecovery';
import { getAllPhotos } from '@/data/photoStorage';
...
export async function runMigrations(): Promise<void> {
  ...
  try {
    await recoverOrphanedProjectPhotos();
  } catch {
    // Recovery should not block app startup.
  }

  try {
    await getAllPhotos();
  } catch {
    // Prune orphaned metadata entries on startup.
  }
}
